...
public class CardDatabase : MonoBehaviour {
    ...
    public void crearCanvasCartas() {
        ...
        for (int i = 0; i < gameManager.jugadores.Count; i++) {
            ...
            Card card = tarjeta.AddComponent<Card>();
            cardList.Add(card);
            tarjeta.AddComponent<Image>().color = Color.black;
            RectTransform rectTransformTarjetas = tarjeta.GetComponent<RectTransform>();
            rectTransformTarjetas.anchoredPosition = new Vector2(card.pos_CCard[0], card.pos_CCard[1]);
            ...
            rectTransformCifraDinero.anchoredPosition = new Vector2(-0.7f, -0.4f);
            ...
            card.playerMoneyText = cifraDineroTexto;
    ...
    public static void updateCardList(int whosTurn, bool inicial) {
        ...
        for (int i = 0; i < cardList.Count; i++) {
            if (i == 0) {
                ...
                cardList[i].pos_CCard = pos_PlayingCard;
                rectTransformTarjetas.anchoredPosition = new Vector2(cardList[i].pos_CCard[0], cardList[i].pos_CCard[1]);
            } else {
                cardList[i].pos_CCard =
                    new float[]
                    {
        ...
        for (int i = 0; i < cardList.Count; i++) {
            Debug.Log("Posicion de la carta player " + i + " = [" + cardList[i].pos_CCard[0] + "," + cardList[i].pos_CCard[1] + "]");
        }
    }
}
